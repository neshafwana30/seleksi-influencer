...
async def scrape_influencer(page, username, already_known_ids):
    ...
    while scroll_count < MAX_SCROLL_PER_INFLUENCER and not stop_influencer:
        ...
        new_this_pass = 0
        oldest_vid_this_pass = None  # track video terakhir (oldest) di pass ini
        ...
        for link in links:
            ...
            # Update oldest_vid setiap iterasi (karena urutan newest→oldest)
            oldest_vid_this_pass = vid
        ...
        if new_this_pass == 0:
            ...
            # ⚡ LOGIC BARU: Kalau 0 video baru DI PASS INI, check oldest_vid
            # Kalau udah pernah dikenal (ada di already_known_ids) DAN
            # tanggalnya lebih lama dari range, berarti tidak ada video baru
            # yang masuk range -> STOP (jangan scroll terus)
            if oldest_vid_this_pass and oldest_vid_this_pass in already_known_ids:
                dt_oldest = video_id_to_wib(oldest_vid_this_pass)
                if is_older_than_range(dt_oldest):
                    print(f"   🛑 [DEDUPCHECK] Video {oldest_vid_this_pass} ({dt_oldest.date()}) "
                          f"lebih lama dari batas dan sudah pernah dikenal.")
                    print(f"      Tidak ada ID baru di pass ini -> berarti udah selesai. STOP.")
                    stop_influencer = True
                    break
